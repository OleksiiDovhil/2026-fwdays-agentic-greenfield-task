...
| FR-SEARCH-06 | Opt-in "Use my location" button reads geolocation only on explicit click (never on load), sets the active location from the coordinates, and shows a calm inline message if permission is denied or unavailable (BC-PRIVACY-02) | proposed   |
