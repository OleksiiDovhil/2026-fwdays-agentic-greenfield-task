...
- [ ] 1.1 i18n (D7): EITHER reuse the existing `shell.background.label` ("Тло з
...
- [ ] 1.2 `components/providers/WeatherProvider.tsx` (`"use client"`, D1) — a tiny
...
- [ ] 1.3 Mount `<WeatherProvider>` once in `app/layout.tsx` (D1/D8) — **inside** the
...
- [ ] 1.4 `components/forecast/ForecastSection.tsx` PUBLISHES into `WeatherContext`
...
- [ ] 2.1 `lib/animated-bg/day-night.ts` (D4, FR-ANIM-02) — a pure, **total**
...
- [ ] 2.2 `lib/animated-bg/scene.ts` (D3, FR-ANIM-01) — a pure, **total**
...
- [ ] 4.1 `components/shell/WeatherBackground.tsx` (`"use client"`, D2/D8) — REPLACE the
  ...
  internals change).
- [ ] 4.2 Consume `useWeather()` + derive the scene (D2/D3/D4): read the
  ...
  NFR-OBS-01.
- [ ] 4.3 Gradient layer (D2/D6, FR-ANIM-01/02): render a **day OR night base gradient**
...
- [ ] 4.4 Particle layer (D2/D6, FR-ANIM-01): when motion is permitted (§4.5) AND the
...
- [ ] 4.5 Reduced-motion branch (D5, FR-ANIM-03): read `window.matchMedia("(prefers-
...
- [ ] 4.6 Reactivity (D2, FR-ANIM-01): the render is a pure function of `(snapshot,
...
- [ ] 5.1 Unit `lib/animated-bg/day-night.test.ts` (FR-ANIM-02, D4): with INJECTED
...
- [ ] 5.2 Unit `lib/animated-bg/scene.test.ts` (FR-ANIM-01, D3): assert
...
- [ ] 5.3 jsdom `components/shell/WeatherBackground.test.tsx` — gradient renders
...
- [ ] 5.4 jsdom particle-by-category (FR-ANIM-01, D2/D4): with motion permitted
...
- [ ] 5.5 jsdom reduced-motion (FR-ANIM-03, D5): OVERRIDE `window.matchMedia` so
...
- [ ] 5.6 jsdom no-interaction + decorative (FR-ANIM-04, NFR-A11Y-01, D2): assert the
...
- [ ] 5.7 jsdom calm default with no data + console silence (NFR-OBS-01, D2): with a
...
- [ ] 5.8 jsdom `ForecastSection` publishes correctly + its existing tests pass
...
- [ ] 5.9 EVAL (D7) — **LOW-VALUE for a decorative layer.** The only candidate copy is a
...
- [ ] 6.3 Run `npm run test:run` — all unit + jsdom component tests green, INCLUDING the
...
- [ ] 6.6 Run `npx openspec validate add-animated-bg --strict` — zero errors/warnings
...
- [ ] 6.9 RENDER smoke (NOT a DB smoke — there is no DB, ADR-0003; NOT a service smoke —
...
- [ ] 6.10 GATED on 6.9 passing: `npx openspec archive add-animated-bg --yes --skip-specs`
