...
- [x] 1.1 Add a `search` namespace to `lib/i18n/uk.ts` (sibling to `shell.*` —
...
- [x] 1.2 Mirror the same `search.*` keys in `lib/i18n/en.ts` (strict fallback
  ...
  (D8, NFR-I18N-01).
- [x] 1.3 Note in a code comment (D8) that the inert `shell.search.*` slot copy
...
- [x] 2.1 `lib/search/types.ts` (D2) — `GeoSuggestion = { id: string; name: string;
...
- [x] 2.2 `lib/search/validation.ts` (D2, FR-SEARCH-01/02) — a **zod schema for the
...
- [x] 2.3 `lib/search/flag.ts` (D2, FR-SEARCH-02 optional flag) — a pure
...
- [x] 3.1 `app/api/geocode/route.ts` (D1, TC-DATA-01) — export an async
...
- [x] 3.2 Keyless server-side fetch (D1, TC-STACK-03, NFR-COST-01): for a non-empty
...
- [x] 3.3 Parse + map (D1/D2): parse the upstream body with `lib/search`
...
- [x] 3.4 Honest degradation — never a raw 500 (D1/D4, NFR-OBS-01): wrap the body so
...
- [x] 4.1 `components/search/SearchBox.tsx` (`"use client"`, D3) — a single text
  ...
  `search.placeholder`.
- [x] 4.2 Debounce + bounded input (D3, FR-SEARCH-01): each keystroke resets a
  ...
  coordinates/markup.
- [x] 4.3 Latest-wins (D3, spec "Latest-wins"): guard overlapping requests with an
...
- [x] 4.4 Suggestion list + content (D5, FR-SEARCH-02): render results as a single
...
- [x] 4.5 Selection writes the URL via the locked provider (D3, FR-SEARCH-03):
...
- [x] 4.6 Enter on a lone suggestion (D3, FR-SEARCH-04): pressing Enter with exactly
...
- [x] 4.7 Zero results inline (D4, FR-SEARCH-05): when the response is
  ...
  editable.
- [x] 4.8 Failure inline (D4, NFR-OBS-01): a network error, a non-OK handler
...
- [x] 4.9 Accessible combobox/listbox keyboard (D5, NFR-A11Y-01): the input is
  ...
  color).
- [x] 4.10 Opt-in "Use my location" (D6, FR-SEARCH-06, BC-PRIVACY-02): a button
  ...
  crash.
- [x] 4.11 Fill the SearchHero slot (D7, §3a): in `components/shell/SearchHero.tsx`
...
- [x] 5.1 Unit `lib/search/validation.test.ts` (FR-SEARCH-01/02, D2): feed a
...
- [x] 5.2 Unit `lib/search/flag.test.ts` (FR-SEARCH-02, D2): assert `flagEmoji` maps
...
- [x] 5.3 (OPTIONAL) Unit for any extracted debounce/latest-wins helper: if the
  ...
  and note that here. `@trace FR-SEARCH-01`. NOTE: the debounce + AbortController +
  request-id latest-wins logic stays INLINE in `SearchBox` (state/refs are React
  concerns, not framework-free) and is covered by the jsdom tests 5.5/5.6 (one
  request per idle window; stale + post-clear responses discarded) — no separate
  pure helper extracted.
- [x] 5.4 Integration `app/api/geocode/route.test.ts` (TC-DATA-01, NFR-OBS-01, D1):
...
- [x] 5.5 jsdom `components/search/SearchBox.test.tsx` — debounce + suggestions +
  ...
  FR-SEARCH-04`.
- [x] 5.6 jsdom empty-state + latest-wins + failure (FR-SEARCH-05, NFR-OBS-01,
...
- [x] 5.7 jsdom combobox a11y (NFR-A11Y-01, D5): assert the input has `role="combobox"`,
...
- [x] 5.8 jsdom geolocation opt-in (FR-SEARCH-06, BC-PRIVACY-02, D6): mock
...
- [x] 5.9 jsdom oversized/odd input (spec bounded-input): paste a 5,000-char string,
  ...
  `@trace FR-SEARCH-01`.
- [x] 5.10 EVAL `evals/cases/search-copy.eval.ts` (FR-SEARCH-05,
...
- [x] 6.1 Write the section 5 tests FIRST and confirm they FAIL (red), then
  ...
  (Tests were authored red by the test-engineer; confirmed red [4 files failing on
  missing modules] before implementing, then green.)
- [x] 6.2 Run `npm run lint` — zero errors/warnings (incl. the import-boundary check:
  ...
  NFR-I18N-01).
- [x] 6.3 Run `npm run test:run` — all unit + jsdom component + route-handler
  integration tests green. (25 files, 259 tests passed.)
- [x] 6.4 Run `npm run build` — production build succeeds; console clean. Confirm
  ...
  NFR-COST-01). (Route shows as `ƒ /api/geocode` [dynamic, not cached]; grep of
  `.next/static` confirms the host + keys are absent from the client bundle.)
- [x] 6.5 Run `node scripts/check-eval-ratchet.mjs` (the graded-quality bar) — the
  new search eval dimensions are ≥ 90 and the committed score does not drop. (SKIPs:
  no `evals/results/latest.json` yet — the eval-suite judge workflow, maker≠checker,
  grades the copy and writes results; the maker does not self-grade.)
- [x] 6.6 Run `npx openspec validate add-city-search --strict` — zero errors/warnings
  ("Change 'add-city-search' is valid").
- [x] 6.7 Run `npx openspec validate --all --strict` — all specs + changes pass.
  (10 items: 10 passed, 0 failed.)
- [x] 6.8 Update `docs/current-state.md`: stamp date/time (Europe/Kyiv), mark
  ...
  slice writes).
- [x] 6.9 SERVICE/INTEGRATION smoke over MOCKED geocoding (NOT a DB smoke — there is
  ...
  the pass output as the smoke evidence. (SMOKE PASS: route.test.ts [a-c] + SearchBox.test.tsx
  [d-f] + validation/flag unit tests = 4 files / 55 tests green; verbose output captured.)
- [x] 6.10 GATED on 6.9 passing: `npx openspec archive add-city-search --yes
