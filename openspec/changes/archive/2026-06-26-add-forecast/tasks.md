...
- [x] 1.1 Add a `forecast` namespace to `lib/i18n/uk.ts` (sibling to the other
...
- [x] 1.2 Mirror the same `forecast.*` keys in `lib/i18n/en.ts` (strict fallback
  subset, identical key shape). Same calm tone, no exclamation marks (D6, NFR-I18N-01).
- [x] 1.3 Confirm `recharts` is already a dependency (it is, `package.json`); add NO
...
- [x] 2.1 `lib/forecast/types.ts` (D2) — the internal contract crossing the
...
- [x] 2.2 `lib/forecast/validation.ts` (D2, FR-FORECAST-01) — a **zod schema for the
...
- [x] 2.3 `lib/forecast/weather-code.ts` (D2, FR-FORECAST-02) — a pure
...
- [x] 2.4 `lib/forecast/hourly.ts` (D2, FR-FORECAST-03) — a pure `nextHours(hourly:
...
- [x] 3.1 `app/api/forecast/route.ts` (D1, TC-DATA-01) — export an async
...
- [x] 3.2 Keyless server-side fetch (D1, TC-STACK-03, NFR-COST-01): for valid
  ...
  live ONLY here.
- [x] 3.3 Parse + map (D1/D2): parse the upstream body with `lib/forecast`
...
- [x] 4.1 `components/forecast/ForecastSection.tsx` (`"use client"`, D3) — reads
...
- [x] 4.2 Client-driven fetch on location change (D3, FR-FORECAST-05): an effect keyed
...
- [x] 4.3 In-memory location-tagged cache + latest-wins (D3, FR-FORECAST-05): hold the
...
- [x] 4.4 `components/forecast/DayCard.tsx` (D3, FR-FORECAST-02): render one day —
  ...
  console warning.
- [x] 4.5 Render order in `ForecastSection` (D3, FR-FORECAST-02/03/04): compute each
  ...
  (4.7).
- [x] 4.6 `components/forecast/HourlyChart.tsx` + dynamic import (D5, FR-FORECAST-03,
  ...
  warning.
- [x] 4.7 Sunrise + sunset (D3, FR-FORECAST-04): small text under the chart showing
...
- [x] 4.8 States all use the shared `<Notice>` (D4, NFR-OBS-01): no location →
...
- [x] 4.9 Fill the ShellContent forecast slot (D7, §3a): in
...
- [x] 5.1 Unit `lib/forecast/validation.test.ts` (FR-FORECAST-01, D2): feed a
...
- [x] 5.2 Unit `lib/forecast/weather-code.test.ts` (FR-FORECAST-02, D2): assert
...
- [x] 5.3 Unit `lib/forecast/hourly.test.ts` (FR-FORECAST-03, D2): with an INJECTED
...
- [x] 5.4 Integration `app/api/forecast/route.test.ts` (TC-DATA-01, NFR-OBS-01, D1):
...
- [x] 5.5 jsdom `components/forecast/ForecastSection.test.tsx` — renders the view from a
  ...
  FR-FORECAST-04`.
- [x] 5.6 jsdom cache + location change + no-location (FR-FORECAST-05, D3): with a
  ...
  > RESOLVED: the impl is correct (latest-wins identity guard discards B's late
  > response under A). The sub-case (c) assertion was corrected by the orchestrator
  > to use a non-colliding leak sentinel (B `tempMax: 88+i`, `not.toContain("88")`)
  > so it no longer false-positives on A's legitimate "30%" precip. 356/356 green;
  > no test weakened, no implementation change.
- [x] 5.7 jsdom honest degradation + console silence (NFR-OBS-01, D4): a mocked network
...
- [x] 5.8 jsdom `components/forecast/DayCard.test.tsx` formatting (FR-FORECAST-02): a
...
- [x] 5.9 jsdom `components/forecast/HourlyChart.test.tsx` (FR-FORECAST-03,
...
- [x] 5.10 EVAL `evals/cases/forecast-copy.eval.ts` (FR-FORECAST-05, NFR-OBS-01,
...
- [x] 6.1 Write the section 5 tests FIRST and confirm they FAIL (red) for the right
...
- [x] 6.4 Run `npm run build` — production build succeeds; console clean. Confirm the
...
- [x] 6.5 Run `node scripts/check-eval-ratchet.mjs` (the graded-quality bar) — the new
...
- [x] 6.6 Run `npx openspec validate add-forecast --strict` — zero errors/warnings
...
- [x] 6.9 SERVICE/INTEGRATION smoke over MOCKED Open-Meteo forecast payloads (NOT a DB
...
- [x] 6.10 GATED on 6.9 passing: `npx openspec archive add-forecast --yes --skip-specs`
