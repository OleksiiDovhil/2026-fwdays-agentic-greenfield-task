...
- [x] 1.1 Add a `map` namespace to `lib/i18n/uk.ts` (sibling to the other namespaces
...
- [x] 1.2 Mirror the same `map.*` keys in `lib/i18n/en.ts` (strict fallback subset,
  identical key shape). Same calm tone, no exclamation marks (D8, NFR-I18N-01).
- [x] 1.3 Confirm `leaflet` / `react-leaflet` / `@types/leaflet` are already
  dependencies (they are, `package.json`); add NO new dependency.
- [x] 1.4 Document + apply the **Leaflet CSS load strategy** (D7): Leaflet ships
...
- [x] 2.1 `lib/geo/types.ts` (D2) — the internal contract crossing the Server↔Client
...
- [x] 2.2 `lib/geo/reverse-validation.ts` (D2, FR-MAP-03, ADR-0005) — a **zod schema
...
- [x] 2.3 `lib/geo/coordinate-label.ts` (D2, FR-MAP-03) — two pure, **total** helpers:
...
- [x] 3.1 `app/api/reverse-geocode/route.ts` (D1, TC-DATA-01, ADR-0005) — export an
...
- [x] 3.2 Keyless server-side fetch (D1, TC-STACK-04, NFR-COST-01, ADR-0005): for valid
...
- [x] 3.3 Parse + map (D1/D2): parse the upstream body with `lib/geo` (`parseReverseName`)
...
- [x] 3.4 Honest degradation — never a raw 500 (D1/D6, NFR-OBS-01): wrap the body so no
...
- [x] 4.1 `components/map/MapSkeleton.tsx` (D3, FR-MAP-05) — a calm placeholder
...
- [x] 4.2 `components/map/LocationMap.tsx` (`"use client"`, D3, FR-MAP-05) — the wrapper
...
- [x] 4.3 `components/map/LocationMapClient.tsx` (`"use client"`, D3, FR-MAP-01/02):
  ...
  no crash).
- [x] 4.4 Recenter without remount (D3, FR-MAP-01): a child component using `useMap()`
...
- [x] 4.5 Click-to-set (D5, FR-MAP-03, ADR-0005): a child using `useMapEvents({ click })`
...
- [x] 4.6 Attribution always visible bottom-right (D4, FR-MAP-04, TC-MAP-01): show
...
- [x] 4.7 Honest degradation (D6, NFR-OBS-01): a failed reverse lookup is NOT an error
...
- [x] 4.8 Fill the ShellContent map slot (D7, §3a): in
...
- [x] 5.1 Unit `lib/geo/reverse-validation.test.ts` (FR-MAP-03, ADR-0005, D2): feed a
  **real-ish** Nominatim `jsonv2` reverse body (a `display_name` + an `address` with
  ...
  `@trace FR-MAP-03`.
- [x] 5.2 Unit `lib/geo/coordinate-label.test.ts` (FR-MAP-03, D2): assert
...
- [x] 5.3 Integration `app/api/reverse-geocode/route.test.ts` (TC-DATA-01, NFR-OBS-01,
...
- [x] 5.4 jsdom `components/map/LocationMap.test.tsx` — the client-only dynamic wrapper
  ...
  NFR-PERF-03`.
- [x] 5.5 jsdom `components/map/LocationMap.test.tsx` (the 5.4–5.7 component cases
  all live in this single file — there is no separate `LocationMapClient.test.tsx`;
  it imports & renders the `LocationMapClient` default export directly) — bounded +
  marker + attribution over a **mocked react-leaflet** (FR-MAP-01/02/04, TC-MAP-01, D3/D4):
...
- [x] 5.6 jsdom click→setLocation + reverse-name (in `components/map/LocationMap.test.tsx`)
  (FR-MAP-03, NFR-OBS-01, D5): over the
...
- [x] 5.7 jsdom console silence on a healthy session (in `components/map/LocationMap.test.tsx`)
  (NFR-OBS-01, D6): render the map
...
- [x] 5.8 EVAL `evals/cases/map-copy.eval.ts` (FR-MAP-03, NFR-OBS-01, BC-BRAND-01):
...
- [x] 6.6 Run `npx openspec validate add-map --strict` — zero errors/warnings ("Change
  'add-map' is valid").
- [x] 6.7 Run `npx openspec validate --all --strict` — all specs + changes pass.
- [x] 6.8 Update `docs/current-state.md`: stamp date/time (Europe/Kyiv), mark `add-map`
...
- [x] 6.9 SERVICE/RENDER smoke over a MOCKED Nominatim payload (NOT a DB smoke — there is
...
- [x] 6.10 GATED on 6.9 passing: `npx openspec archive add-map --yes --skip-specs` (the
