...
- [ ] 1.1 Add a `jokes` namespace to `lib/i18n/uk.ts` (sibling to `shell.*` —
...
- [ ] 1.2 Mirror the same `jokes.items` array in `lib/i18n/en.ts` (strict
...
- [ ] 1.3 Note in a code comment (D5) that the inert `shell.jokes.placeholder`
...
- [ ] 2.1 `lib/jokes/jokes.ts` — `pickJoke(corpus: readonly string[], key:
...
- [ ] 2.2 `lib/jokes/jokes.ts` — `dailyKey(date: Date): number` (D2): a
...
- [ ] 3.1 `components/jokes/FooterJoke.tsx` (D5, FR-JOKES-01): compute
...
- [ ] 3.2 Wire into the AppFooter jokes slot (D5, §3a): in
...
- [ ] 5.1 Unit `lib/jokes/jokes.test.ts` — DETERMINISM (FR-JOKES-01, D1): for a
  ...
  FR-JOKES-01`.
- [ ] 5.2 Unit — MODULO NORMALISATION incl. negative + out-of-range (FR-JOKES-01,
...
- [ ] 5.3 Unit — EMPTY corpus → undefined, no throw (FR-JOKES-01, NFR-OBS-01, D1):
...
- [ ] 5.4 Unit — MALFORMED entry tolerated (NFR-OBS-01, D1): when the entry at the
...
- [ ] 5.5 Unit — `dailyKey` advances by one per LOCAL day + is NOT toISOString-derived
...
- [ ] 5.6 Unit — NO exclamation marks in any joke (BC-BRAND-01, D3): over
...
- [ ] 5.7 jsdom component `components/jokes/FooterJoke.test.tsx` — renders a joke
  (FR-JOKES-01, NFR-I18N-01, D5): with a populated corpus, render `FooterJoke` (or
  `AppFooter`) and assert the rendered joke text EQUALS the expected
  `corpus[dailyKey(today) mod N]` Ukrainian entry (so it comes from i18n, not a
  hardcoded literal) and that the joke node carries the accessible label from
  `shell.jokes.label`; spy on `console.error`/`console.warn` and assert NEITHER was
  called. `@trace FR-JOKES-01, NFR-I18N-01`.
- [ ] 5.8 jsdom component — EMPTY corpus omits the line (NFR-OBS-01, D4): with an
  empty corpus (mock/inject `jokes.items` = `[]`, or drive `FooterJoke` with an
  empty corpus), render the footer and assert the joke `<p>` / `data-slot="jokes"`
  text is NOT present (the line is omitted, not blank), that the REST of the footer
  (the Open-Meteo / OpenStreetMap credits and the privacy line) STILL renders, and
  that the console stays silent (no warning/error). `@trace NFR-OBS-01`.
- [ ] 5.9 EVAL `evals/cases/jokes-quality.eval.ts` (FR-JOKES-01, BC-BRAND-01,
...
- [ ] 6.2 Run `npm run lint` — zero errors/warnings.
- [ ] 6.3 Run `npm run test:run` — all unit + jsdom component tests green.
- [ ] 6.4 Run `npm run build` — production build succeeds; console clean (no
...
- [ ] 6.5 Run `npx openspec validate add-bottom-jokes --strict` — zero
  errors/warnings ("Change 'add-bottom-jokes' is valid").
- [ ] 6.6 Run `npx openspec validate --all --strict` — all specs + changes pass.
- [ ] 6.7 Update `docs/current-state.md`: stamp date/time (Europe/Kyiv), mark
...
- [ ] 6.8 SERVICE / RENDER smoke (NOT a DB smoke — there is no DB, ADR-0003), step
...
- [ ] 6.9 GATED on 6.8 passing AND the `jokes-quality` eval meeting the bar (every
