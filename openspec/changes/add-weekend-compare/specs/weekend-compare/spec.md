...
<!--
This delta restates the baseline `openspec/specs/weekend-compare/spec.md` contract
(adopted at G2, 7 requirements — FR-COMPARE-01..03 + the NFR-A11Y-01 / NFR-I18N-01 /
NFR-OBS-01 cross-cutting requirements that travel with this slice) verbatim as ADDED
requirements, for the record and so `openspec validate add-weekend-compare --strict`
can validate the change against a delta. Archive runs with `--skip-specs` because the
baseline spec already holds this content (the requirements are NOT re-applied via
OpenSpec Option B). Keep this file in sync with the baseline if the baseline changes.

CROSS-SLICE NOTE (design.md D1/D3/D4, no new ADR — applies the locked decisions):
this capability is a PURE COMPOSER over three LOCKED upstreams and writes no new
cross-cutting machinery:
  - the active location — `useLocation()` (LocationProvider): a "pin" pins the CURRENT
    active location; "make active" calls `setLocation(city)` (the same setter
    city-search/map write). It does NOT re-parse the URL.
  - the `forecast` capability — each pinned city's weekend data comes from the SAME
    keyless `app/api/forecast?lat=&lon=` route handler (reused — NO new endpoint), the
    validated `Forecast`/`DailyForecast` shape, and the `nextHours`-free daily path;
    all pinned cities are fetched IN PARALLEL (`Promise.all`, no waterfall) and cached
    per city in memory (ADR-0003).
  - the `comfort-score` capability — per-day `comfortScore(toComfortInput(day))` and
    the `upcomingWeekend(days)` Saturday/Sunday selection (by the location-local
    `time` date, never `toISOString`/the viewer's clock); the comfort number is shown
    via the locked `ComfortBadge`. This capability does NOT recompute weather or
    scoring.
Pinned-city state lives in memory ONLY (no DB, no cookies/localStorage — ADR-0003) and
resets on reload. It fills the shell's `ShellContent` compare slot; it does NOT edit
`app/page.tsx` (§3a serialize point).
-->
