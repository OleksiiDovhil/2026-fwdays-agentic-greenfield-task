...
<!--
This delta restates the baseline `openspec/specs/forecast/spec.md` contract
(adopted at G2, 8 requirements) verbatim as ADDED requirements, for the record
and so `openspec validate add-forecast --strict` can validate the change against a
delta. Archive runs with `--skip-specs` because the baseline spec already holds
this content (the requirements are NOT re-applied via OpenSpec Option B). Keep this
