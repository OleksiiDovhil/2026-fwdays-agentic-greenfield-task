...
Turn a single day's weather into a calm, glanceable verdict for trip planning:
a 0–100 comfort number plus one short Ukrainian sentence explaining it. The
score is computed by a pure, total function (`lib/scoring/comfort.ts`) so it is
fully unit-testable and never throws, then surfaced as a colored badge per day
and as a highlighted upcoming-weekend summary at the top of the forecast grid.
This capability owns the scoring math and its presentation contract; it does not
fetch weather data or own the forecast layout.
...
## Exclusions (intentionally out of scope for MVP)

The following are intentionally unsupported and SHALL NOT be reported as defects:

- **No emoji in comfort output.** The `rationale` and badges use plain Ukrainian
  text and color only; emoji are intentionally excluded (FR-COMFORT-03).
- **No multi-weekend or arbitrary-date comfort.** Only the single upcoming
  weekend (next Sat+Sun) is highlighted; picking other date ranges is out of
  scope (FR-COMFORT-05).
- **No data fetching here.** This capability does not call Open-Meteo; it
  consumes already-fetched daily data owned by the `forecast` capability — all
  five scoring factors (feels-like temperature, precipitation probability, wind,
  cloud cover, UV index) and each day's location-local `daily.time` date are
  supplied by `forecast`'s single request (FR-FORECAST-01), not fetched or
  recomputed here.
- **No per-hour comfort score.** Comfort is computed per day (and the weekend
  average), not for individual hours.
- **No persistence or history.** Scores are computed on the fly; nothing is
  stored server-side (keyless, stateless — ADR-0003).
- **No localisation beyond UA + EN.** Rationale is Ukrainian; only UA + EN
  labels exist; other languages are out of scope.
- **No alternative comfort models or user-tunable weights in MVP.** The scoring
  formula is fixed in `lib/scoring/comfort.ts`; user-adjustable preferences are
  out of scope.
