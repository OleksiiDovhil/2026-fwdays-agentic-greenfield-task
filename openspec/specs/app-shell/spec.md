...
The app shell is the foundational single-page frame that every other capability
renders inside. It owns the top bar (logo and theme indicator), the responsive
main content area, the first-load empty state (hero copy plus a centered city
search), and the shared calm inline error/empty-state pattern that all other
capabilities reuse instead of generic 500s or silent blanks. It also owns the
centralized Ukrainian-first UI string layer and keeps the active location and
view state in the URL and in memory only — never in server-side storage or
application-set cookies. As the owner of URL-as-state, the shell decodes and
validates the raw (untrusted) location query parameters into the in-memory
active-location state that downstream capabilities consume; a malformed,
out-of-range, or partial query string degrades to the first-load empty state.
