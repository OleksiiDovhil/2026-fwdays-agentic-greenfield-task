...
Let an anonymous visitor find a place by typing its name or by opting in to
browser geolocation, then make that place the active location for the rest of
the app. Search is keyless (Open-Meteo geocoding), Ukrainian-first, and calm:
zero results and every failure are surfaced inline, never as an error toast or
a 500. The chosen location is reflected in the URL so a result is shareable.

### Pinned constants and conventions

To keep every timing-, validation-, and copy-sensitive behavior objectively
pass/fail (mirroring the concrete numbers in the sibling `forecast` spec):

- **Debounce interval:** `300 ms`. A geocoding request fires only after the
  input has been idle for at least 300 ms since the last keystroke.
- **Maximum query length:** `120` characters. Input is hard-capped at 120
  characters (a `maxlength`-equivalent truncation); a query longer than 120
  characters before capping is truncated to 120 and only the truncated value is
  sent to the geocoding API.
- **Coordinate contract for `lat`/`lon`:** a value is _valid_ only when it
  parses as a finite JSON number (period `.` as the decimal separator, never a
  locale comma) with `lat` in the closed range `[-90, 90]` and `lon` in the
  closed range `[-180, 180]`. Any other form — non-numeric, out of range,
  empty, or a locale-variant comma decimal such as `50,45` — is _invalid_.
- **Geocoding payload contract:** the response body is _valid_ only when it
  parses as JSON matching the zod schema `{ results?: Array<{ name: string;
  latitude: number; longitude: number; country?: string; country_code?:
  string; admin1?: string }> }`. A `200` response whose body fails this schema
  is treated exactly like a failed fetch (see "Failures and malformed payloads
  degrade calmly"). An absent or empty `results` array is _valid_ and means
  zero results, not a failure.
- **Latest-wins ordering:** geocoding requests are debounced but can still
  overlap; the UI renders results from the most recent issued query only. A
  superseded (earlier) request that resolves after a newer one SHALL NOT
  replace the newer query's suggestions or empty state.
- **Inline copy:** all inline messages (zero results and every failure state)
  are Ukrainian-first, sourced from the centralised i18n strings, with a calm
  tone and no exclamation marks (NFR-I18N-01, BC-BRAND-01).

> **FR literal reconciliation (FR-SEARCH-05).** FR-SEARCH-05 and the product
> brief quote the zero-results string in English as "Nothing found". This spec
> asserts the Ukrainian literal "Нічого не знайдено" because the product is
> Ukrainian-first (NFR-I18N-01, BC-BRAND-01): the FR fixes the _meaning_, the
> spec fixes the _shipped literal_. This deliberate override is logged in
> `docs/requirements-clarifications.md` alongside the FR-SEARCH-06
> reconciliation; the English "Nothing found" remains the `en.ts` fallback.
