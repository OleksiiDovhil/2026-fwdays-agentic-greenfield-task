...
The top-clock capability renders a compact, accessible local-time clock in the
application header (FR-CLOCK-01). It shows the visitor's own device-local time
and keeps ticking live while the page stays open. As a client-only widget it
must expose an accessible name, satisfying the accessibility bar that travels
with it (NFR-A11Y-01), and as a live-updating header widget it must never cause
layout shift as the digits change — a Core Web Vitals constraint that supports
the Lighthouse Performance bar (NFR-PERF-02). It carries no weather data, no
network calls, and no persistence.
...
These items are intentionally unsupported in the MVP so testers do not report
them as defects:

- Showing the time of the active weather location or any time zone other than
  the visitor's device-local time.
- A user-selectable time-zone picker, 12/24-hour toggle, or locale/format
  selector.
- Showing the date, day of week, or seconds-precision toggles beyond the live
  local time.
- Persisting any clock preference (no cookies, no storage; the product is
  keyless and stateless).
- Server-side or network-sourced time (NTP, API time); the clock reads only the
  client device clock.
- Sub-second precision or millisecond display; updating at least once per
